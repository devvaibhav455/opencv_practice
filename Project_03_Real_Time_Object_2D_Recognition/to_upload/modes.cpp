...
int basic_training_mode_from_directory(){
  ...
  while( (dp = readdir(dirp)) != NULL ) {
    ...
    if( strstr(dp->d_name, ".jpg") ||
      ...
      std::vector<std::vector<double>> vofv;

      find_valid_obb_vertices_and_centroid(labelImage, contours, valid_vtx_vec, valid_centroid_vec, valid_alpha_vec, vofv, 15);
      std::cout << "Valid obb are: " << valid_alpha_vec.size() << std::endl;

      if(valid_alpha_vec.size() != 1){
        std::cout << "Valid obb are: " << valid_alpha_vec.size() << " , skipping this image" << std::endl;
        continue;
      }
      feature_vec = vofv[0];
...
int object_recognition_mode(int &source, cv::Mat &frame){
  ...
  cv::String windowName_target_image = "Target Image with label"; //Name of the window
  cv::namedWindow(windowName_target_image); // Create a window

  cv::String windowName_original_image = "Original Image"; //Name of the window
  cv::namedWindow(windowName_original_image); // Create a window
  ...
  while (true) {
    ...
    cv::imshow(windowName_original_image, frame);
    

    // Calculate feature vector for the target image
    std::string target_image_label;
    std::vector<std::vector<float>> target_image_feature_vec;
    cv::Mat src_image_copy_valid_boxes;
    std::vector<cv::Point2f> valid_centroid_vec;
    calc_feature_vector(frame, target_image_feature_vec, src_image_copy_valid_boxes, valid_centroid_vec);
    ...
    std::cout << "Not printing the target image feature vector inside mode function" << std::endl;
    // for (auto it:target_image_feature_vec[0]){
    //   std::cout << it << " ";
    // }
    // std::cout << std::endl;

    for (int i = 0; i < target_image_feature_vec.size() && target_image_feature_vec.size() != 0; i++ ){
      // char *target_filename_char_star; 
      
      one_object_classifier(frame, target_image_feature_vec[i], files_in_csv, feature_vectors_from_csv, standard_deviation_vec, target_image_label);
      int fontFace = 0;
      double fontScale = 1;
      int thickness = 2;
      int baseline=0;
      ...
      cv::Point textOrg(valid_centroid_vec[i].x - 70 , valid_centroid_vec[i].y - 30);
      putText(src_image_copy_valid_boxes, target_image_label, textOrg, fontFace, fontScale,cv::Scalar(0,0,255), thickness, 8);
    }
...
int knn_classifier(int &k, char * target_filename_char_star){
  ...
  // Calculate feature vector for the target image
  std::string target_image_label;
  std::vector<std::vector<float>> vofv;
  std::vector<float> target_image_feature_vec;
  cv::Mat src_image_copy_valid_boxes;
  std::vector<cv::Point2f> valid_centroid_vec;
  calc_feature_vector(frame_target, vofv, src_image_copy_valid_boxes,valid_centroid_vec);
  if (vofv.size() != 1){
    std::cout << "KNN approach currently works with just one valid OBB!" << std::endl;
    exit(-1);
  }
  target_image_feature_vec = vofv[0];
  ...
  for (int i = 0; i < unique_labels.size(); i++){
    ...
    // calc_manhattan_l1_dist_vec(target_image_feature_vec, vofv_within_class, sdv_all_class, dmv_within_class);
    // calc_chisquare_dist_vec(target_image_feature_vec, vofv_within_class, sdv_all_class, dmv_within_class);
...
int testing_mode(int &source, cv::Mat &frame){
  ...
  while (true) {
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //                  Applying thresholding to the original frame                                                                 //
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    cv::Mat cleaned_image(frame.size(), CV_8UC1), temp_cleaned(frame.size(), CV_8UC1);
    grassfire_tf(thresholded_image,temp_cleaned, 8, 0, 20); // Perform erosion with 8 connectedness for at most 20 times
    // grassfire_tf(temp_cleaned,cleaned_image, 4, 1, 20);
    ...
    // Find the oriented bounding box for all the contours in the image.
    find_obb_vertices_and_centroid(labelImage, contours, all_vtx_vec, all_centroid_vec, all_alpha_vec);
    std::cout << "Num obb are: " << all_alpha_vec.size() << std::endl;
    cv::Mat frame_copy_all_boxes;
    frame.copyTo(frame_copy_all_boxes);
    ...
    // Find valid obb from all the contours in the image
    find_valid_obb_vertices_and_centroid(labelImage, contours, valid_vtx_vec, valid_centroid_vec, valid_alpha_vec, vofv_double, 15);

    std::cout << "Valid obb are: " << valid_alpha_vec.size() << std::endl;
    
    if (vofv_double.size() != 0){
      std::vector<std::vector<float>> vofv_float(vofv_double.size(), std::vector<float>(vofv_double[0].size()));
  
      for (int i = 0; i < vofv_double.size(); i++) {
        for (int j = 0; j < vofv_double[i].size(); j++) {
            vofv_float[i][j] = static_cast<float>(vofv_double[i][j]);
        }
      }

      if (key_pressed == 'n' && vofv_float.size() == 1){
        //Save image if 'n' is pressed 
        ...
        append_image_data_csv("training_data.csv", user_input_label , vofv_float[0], 0);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//                                PLEASE IGNORE CODE BELOW THIS LINE. KEPT FOR MY PERSONAL REFERENCE                                             //  
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
